...
pub(crate) fn generate_link_path_permutations(
    ...
    match (source, target, predicate) {
        ...
        (Some(source), None, None) => {
            ...
            Ok(vec![LinkPermutation::new(source.clone(), wildcard)])
        }
        (None, Some(target), None) => {
            // Target -> * -> LinkExpression
            Ok(vec![LinkPermutation::new(target.clone(), wildcard)])
        }
        (None, None, Some(predicate)) => {
            // Predicate -> * -> LinkExpression
            Ok(vec![LinkPermutation::new(predicate.clone(), wildcard)])
        }
        (None, None, None) => Err(WasmError::Host(String::from("Link has no entities"))),
    }
}
...
pub(crate) fn get_link_permutation_by(triple: Triple) -> LinkPermutation {
    ...
    match (source, target, predicate) {
        ...
        (Some(source), Some(target), _) => LinkPermutation::new(source, target),
        //Query with source + predicate
        //Here target is unknown and thus the value the zome caller is looking for
        (Some(source), None, Some(predicate)) => LinkPermutation::new(source, predicate),
        (None, Some(target), Some(predicate)) => LinkPermutation::new(target, predicate),
        //Look for all links with the given source
        (Some(source), None, None) => LinkPermutation::new(source, wildcard),
        (None, Some(target), None) => LinkPermutation::new(target, wildcard),
        (None, None, Some(predicate)) => LinkPermutation::new(predicate, wildcard),
        ...
        (None, None, None) => LinkPermutation::new(wildcard.to_string(), wildcard),
    }
}
...
mod tests {
    ...
    fn generate_link_path_permutations_works() {
        ...
        assert_eq!(
            result[0],
            LinkPermutation::new(TRIPLE_SOURCE.to_string(), WILDCARD.to_string())
        );
        assert_eq!(
            result[1],
            LinkPermutation::new(TRIPLE_TARGET.to_string(), WILDCARD.to_string())
        );
        assert_eq!(
            result[2],
            LinkPermutation::new(TRIPLE_PREDICATE.to_string(), WILDCARD.to_string())
        );
        assert_eq!(
            result[3],
            LinkPermutation::new(TRIPLE_SOURCE.to_string(), TRIPLE_TARGET.to_string())
        );
        assert_eq!(
            result[4],
            LinkPermutation::new(TRIPLE_SOURCE.to_string(), TRIPLE_PREDICATE.to_string())
        );
        assert_eq!(
            result[5],
            LinkPermutation::new(TRIPLE_TARGET.to_string(), TRIPLE_PREDICATE.to_string())
        );
    ...
    fn get_link_permutation_by_triple_works() {
        ...
        assert_eq!(
            result,
            LinkPermutation::new(WILDCARD.to_string(), WILDCARD.to_string())
        );
    }
}
