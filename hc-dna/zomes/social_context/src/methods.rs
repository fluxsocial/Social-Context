...
use crate::utils::{
    generate_link_path_permutations, get_link_permutation_by, get_wildcard, LinkPermutation,
};
use crate::{
    AddLinkInput, AgentReference, GetLinks, IndexStrategy, LinkExpression, SocialContextDNA,
    UpdateLinkInput, ACTIVE_AGENT_DURATION, ENABLE_SIGNALS, ENABLE_TIME_INDEX,
};
...
impl SocialContextDNA {
    ...
    pub fn add_active_agent_link() -> SocialContextResult<Option<DateTime<Utc>>> {
        ...
        debug!(
            "Social-Context.add_active_agent_link: Got recent agents: {:#?}",
            recent_agents
        );
    ...
    pub fn index_link(input: AddLinkInput) -> SocialContextResult<()> {
        ...
        let link_indexes = match input.index_strategy {
            ...
            IndexStrategy::Simple => vec![LinkPermutation::new(
                input.link_expression.data.source.clone().ok_or(
                    SocialContextError::RequestError("Expected source with simple index strategy"),
                )?,
                input.link_expression.data.predicate.clone().ok_or(
                    SocialContextError::RequestError(
        ...
        for link_index in link_indexes {
            if *ENABLE_TIME_INDEX {
                ...
                hc_time_index::index_entry(
                    link_index.root_index,
                    input.link_expression.clone(),
                    link_index.tag,
                )?;
            } else {
    ...
    pub fn get_links(get_links: GetLinks) -> SocialContextResult<Vec<LinkExpression>> {
        ...
        if *ENABLE_TIME_INDEX {
            ...
            if get_links.from_date.is_some() && get_links.until_date.is_some() {
                ...
                let unix = DateTime::<Utc>::from_utc(NaiveDateTime::from_timestamp(0, 0), Utc);
                Ok(hc_time_index::get_links_and_load_for_time_span::<
                    LinkExpression,
                >(
                    link_query_elements.root_index,
            ...
            SocialContextDNA::make_simple_link_query(
                Path::from(link_query_elements.root_index).hash()?,
                Some(link_query_elements.tag),
            )
        }
    }
    ...
    pub fn remove_link(link: LinkExpression) -> SocialContextResult<()> {
        ...
        if *ENABLE_TIME_INDEX {
            ...
            let wildcard = get_wildcard();
            link_indexes.push(LinkPermutation::new(
                wildcard.to_string(),
                wildcard.to_string(),
            ));
