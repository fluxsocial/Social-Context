...
orchestrator.registerScenario("basic link testing", async (s, t) => {
  ...
  await alice_sc_happ.cells[0].call("social_context_link_store", "add_link", {subject: "root://hash", object: "explang://exp", predicate: null})

  //Emulate creating language -> expression link
  await alice_sc_happ.cells[0].call("social_context_link_store", "add_link", {subject: "explang://explang", object: "explang://exp", predicate: null})

  //Emulate creating agent -> expression link
  await alice_sc_happ.cells[0].call("social_context_link_store", "add_link", {subject: "did://alice", object: "explang://exp", predicate: null})

  //Emulate creating agent -> language link
  await alice_sc_happ.cells[0].call("social_context_link_store", "add_link", {subject: "did://alice", object: "explang://explang", predicate: null})
  ...
  const get_others = await alice_sc_happ.cells[0].call("social_context_link_store", "get_others", {subject: "chunk://hc-agent://hc-agent?chunk=0"})
  t.deepEqual(get_others.length, 1);
  console.log("Links on alice first chunk", get_others);
})
