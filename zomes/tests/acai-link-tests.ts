...
orchestrator.registerScenario("test link update", async (s, t) => {
  const [alice, bob] = await s.players([conductorConfig, conductorConfig])
  ...
  //Test case where subject object and predicate are given
  await alice_sc_happ.cells[0].call("social_context_acai", "add_link", { data: {source: "subject-og", target: "object-og", predicate: "predicate-og"},
  author: {did: "test1", name: null, email: null}, timestamp: "iso8601", proof: {signature: "sig", key: "key"} })

  await alice_sc_happ.cells[0].call("social_context_acai", "add_link", { data: {source: "subject-og-2", target: "object-og", predicate: "predicate-og-2"},
  author: {did: "test1", name: null, email: null}, timestamp: "iso8601", proof: {signature: "sig", key: "key"} })

  await alice_sc_happ.cells[0].call("social_context_acai", "update_link", { source: { data: {source: "subject-og", target: "object-og", predicate: "predicate-og"},
  author: {did: "test1", name: null, email: null}, timestamp: "iso8601", proof: {signature: "sig", key: "key"} }, 
    target: {  data: {source: "subject-update", target: "object-og", predicate: "predicate-update"},
    author: {did: "test1", name: null, email: null}, timestamp: "iso8601", proof: {signature: "sig", key: "key"} } })

  const subj_links = await alice_sc_happ.cells[0].call("social_context_acai", "get_links", {source: "subject-og", target: null, predicate: null})
  t.deepEqual(subj_links.length, 0);
  console.log("subject links", subj_links);

  const object_og_links = await alice_sc_happ.cells[0].call("social_context_acai", "get_links", {source: "object-og", target: null, predicate: null})
  t.deepEqual(object_og_links.length, 2);
  console.log("object links", object_og_links);

  const pred_links = await alice_sc_happ.cells[0].call("social_context_acai", "get_links", {source: null, target: null, predicate: "predicate-og"})
